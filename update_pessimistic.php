<?php
  ...
  $stmt = $db->prepare('SELECT goods.*, users_e.email, now() > last_edit_starts_at + INTERVAL 5 MINUTE AS edit_expired FROM goods LEFT JOIN users_e ON users_e.id=goods.last_edit_starts_by_user WHERE id=:id');
