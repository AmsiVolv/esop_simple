<?php
  ...
  if (!empty($_POST)) {
    ...
    //uložíme uživatele do DB
        $stmt = $db->prepare("INSERT INTO users_e(id, email, password, role) VALUES (3, ?, ?, 'user')");
    ...
    $stmt = $db->prepare("SELECT id FROM users_e WHERE email = ? LIMIT 1"); //limit 1 jen tu jen jako výkonnostní optimalizace
    $stmt->execute([$email]);
